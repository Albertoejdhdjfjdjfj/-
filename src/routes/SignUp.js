 ...
 export default function SignUp(){
  ...
  <div>
    ...
    <div>SignUp</div>
     ...
     <input onChange={(e)=>{setRepeatPassword(e.target.value)}} type="password" min='6' max='20' placeholder="Repeat password"/>
  ...
  async function signUp(){
    if(password===repeatpassword){
     ...
     await fetch(`http://localhost:3001/users?email=${email}&password=${password}`)
      ...
      .then((users)=>{ if(users.length!==0){setEror('Такой пользователь уже существует')}
      else{
